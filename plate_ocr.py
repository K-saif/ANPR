...
def process_cropped_plates(
    cropped_folder: str, 
    output_file: Optional[str] = None
) -> List[dict]:
    """
    Process all cropped plate images in a folder.
    
    Args:
        cropped_folder: Path to folder containing cropped plate images
    ...
    # Get all jpg files
    image_files =  os.listdir(cropped_folder)
    
    print(f"Processing {len(image_files)} plate images...")
    
    for img_file in sorted(image_files):
        img_path = os.path.join(cropped_folder, img_file)
        result = ocr.extract_text_from_file(img_path)
        result['filename'] = img_file
        results.append(result)
        
        print(f"{img_file}: {result['text']} (conf: {result['confidence']})")
    
    # Save to CSV if output file specified
    if output_file:
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=['filename', 'text', 'english', 'arabic', 'confidence'])
            writer.writeheader()
            for r in results:
                writer.writerow({
                    'filename': r['filename'],
...
if __name__ == "__main__":
    # Test on existing cropped plates
    import config
    
    results = process_cropped_plates(
        config.CROPPED_FOLDER,
        output_file="/home/medprime/Music/ANPR/ocr_results.csv"
    )
